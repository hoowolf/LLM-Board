...
def download_and_save_activity_data(repo_name, org_name, token=None):
    url = f"https://oss.open-digger.cn/github/{org_name}/{repo_name}/activity.json"
    headers = {}
    if token:
        headers['Authorization'] = f'token {token}'
    ...
    if response.status_code == 200:
        data = response.json()
        file_path = os.path.join(os.getcwd(), f"{repo_name}_activity.json")
...
def merge_all_data_to_json(org_names, token=None):
    ...
    for org_name in org_names:
        ...
        for repo_name in repo_names:
            repo_data = download_and_save_activity_data(repo_name, org_name, token)
            if repo_data is not None:
                org_data[repo_name] = repo_data
        
        all_data[org_name] = org_data
    
    output_file = 'all_activity_data.json'
